...
from textual.widgets import Header, Footer, Input, TabPane, TabbedContent, Button, DataTable, Select
...
class Calculators(App):
    ...
    @on(Button.Pressed, "#calc") #the "#calc" is to specify that the button is specifically for that id # pylint: disable=line-too-long
    ...
    def format_results(self, results, tool_name) -> None:
        ...
        Returns:
            None
        """
        results_datatable: DataTable = self.query_one(f"#{tool_name}_results", DataTable)
        rows: list = []
